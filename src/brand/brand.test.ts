...
function snap(p: Partial<StyleSnapshot>): StyleSnapshot {
  return {
    tag: 'div', area: 100, color: 'rgb(20, 20, 20)', background: 'rgba(0, 0, 0, 0)',
...
test('brand: recovers type roles and scales', () => {
  const brand = assembleBrand({ light: lightSite(), dark: lightSite(), defaultBackground: 'rgb(255, 255, 255)' });
...
test('brand: emits both palettes when the site themes light and dark', () => {
  ...
  const brand = assembleBrand({ light, dark, defaultBackground: 'rgb(255, 255, 255)' });
...
test('brand: emits a single palette when the site ignores the scheme', () => {
  ...
  const brand = assembleBrand({ light: only, dark: only, defaultBackground: 'rgb(255, 255, 255)' });
